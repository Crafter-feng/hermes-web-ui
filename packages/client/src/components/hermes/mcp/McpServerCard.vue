...
const emit = defineEmits<{
  ...
  showTools: [server: McpServerInfo]
}>()
...
function statusLabel(server: McpServerInfo) {
  if (server.enabled === false) return t('mcp.disabledStatus')
...
const isLoadingTools = computed(() => props.loadingTools === props.server.name)
</script>

<template>
  <div class="mcp-card" :class="{ disconnected: !server.connected, disabled: server.enabled === false }">
    <!-- 第一行：标题 + 标签 -->
    ...
    <div class="card-body">
      ...
      <div v-if="server.tools > 0" class="tools-list">
        <span
          v-for="tool in tools.slice(0, 20)"
          :key="tool.name"
          class="tool-tag"
        ...
        <span v-if="tools.length > 20" class="tool-tag tool-tag-more">
          +{{ tools.length - 20 }} {{ t('mcp.more') }}
        </span>
        <span v-if="isLoadingTools" class="tool-tag tool-tag-loading">{{ t('mcp.loading') }}</span>
      </div>
...
.tool-tag {
  ...
  &-loading {
    background: rgba(var(--text-muted-rgb, 128,128,128), 0.1);
    color: $text-muted;
    font-style: italic;
  }
}
